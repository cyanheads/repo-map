...
├── src/
    ...
    │   │   └── Description: Provides language-specific functions to parse source code files, extracting structural elements like imports, docstrings, and function
    │   ├── config.py (Python)
    ...
    │   │   ├── Security Assessment: "This file handles API keys; ensure that sensitive fields are loaded securely from environment variables and never hardcoded."
    │   │   ├── Critical Dependencies:
    ...
    │   │   ├── Developer Consideration: "All LLM calls must be wrapped in the global `API_SEMAPHORE` to respect rate limits; failure to do so will lead to API throttling."
    │   │   ├── Maintenance Flag: Stable
    │   │   ├── Architectural Role: Service Layer
    │   │   ├── Code Quality Score: 8/10
    │   │   ├── Refactoring Suggestions: "Abstract the LLM provider logic (e.g., OpenAI vs. Anthropic) into a factory pattern or separate classes for easier future expansion."
